...
#+begin_src haskell :tangle example1.hs
  ...
  main :: IO ()
  main = do
    let config = mkConfig "https://my-erpnext.frappe.cloud/api" "api-key" (mkSecret "api-secret")
    manager <- newTlsManager
    response <- getDoc manager config "Company A" :: IO (ApiResponse Customer)
    case response of
...
First, define these variables in a =.env= file:
...
#+begin_src haskell
  ...
  res <- getDoc @DocType manager config "Task"
#+end_src
